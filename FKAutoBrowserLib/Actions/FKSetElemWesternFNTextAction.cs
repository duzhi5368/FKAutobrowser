...
{
    /// <summary>
    /// 为当前元素输入一个西方的FirstName
    /// Types a random first name into the current element
    /// </summary>
    class FKSetElemWesternFNTextAction : FKBaseAction
    {
        public FKSetElemWesternFNTextAction()
        {
        }

        public FKSetElemWesternFNTextAction(string selector)
        {
            Selector = selector;
        }
            ...
            try
            {
                ...
                foreach (IFKWebElement element in elements)
                {
                    try
                    {
                        ...
                        string name = random.GetWesternFirstName();
                ...
                PostActionMessage = String.Format(
                        "向元素输入随机西方FirstName, {0} 个元素成功, {1} 个元素失败",
                        succeededElements,
                        failedElements);
                Success = true;
            }
